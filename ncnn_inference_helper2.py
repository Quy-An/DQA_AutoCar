...
# Or implement manually: def sigmoid(x): return 1.0 / (1.0 + np.exp(-x))


class NCNN_YOLO_Processor:
    def __init__(self, param_path, bin_path, input_size=320):
        # Sử dụng khối 'with' để đảm bảo tài nguyên NCNN được quản lý đúng cách
        self.net = ncnn.Net()
        self.net.opt.num_threads = 4 # Use multiple threads for inference
        self.net.opt.use_vulkan_compute = False # Tắt Vulkan cho Raspberry Pi
        ...
        # This matches cv2.dnn.blobFromImage(..., 1/255.0, ...) with mean (0,0,0).
        ...
        self.num_proto_channels = 256 # From Segment layer args in YAML (number of channels in prototype output == num prototypes)
        ...
        # The format and values of anchors are model/training specific.
    ...
    def preprocess(self, image_bgr_np):
        ...
        # Resize image to model input size (320x320)
        img_resized = cv2.resize(image_bgr_np, (self.input_size, self.input_size))
        # Convert to NCNN Mat (BGR format)
    ...
    def postprocess(self, ncnn_outputs, original_width, original_height, conf_threshold, nms_threshold=0.45):
        ...
        print(f"Helper: Expected detection output shape (38, 2100). Expected segmentation output shape ({self.num_proto_channels}, H_proto, W_proto).")
        ...
        detection_output_np = np.array(detection_output_mat)
        ...
        # Verify expected shapes (add checks here based on what you find from conversion source)
        if detection_output_np.shape != (38, 2100):
             print(f"Helper: Warning: Unexpected detection output shape {detection_output_np.shape}. Expected (38, 2100). Postprocessing might fail.")

        # Determine prototype spatial dimensions
        if segmentation_output_np.ndim >= 3 and segmentation_output_np.shape[0] == self.num_proto_channels:
             proto_height = segmentation_output_np.shape[1]
             proto_width = segmentation_output_np.shape[2]
             print(f"Helper: Detected prototype spatial dimensions: {proto_width}x{proto_height}")
        else:
             print(f"Helper: Warning: Unexpected segmentation output shape {segmentation_output_np.shape} for mask generation.")
             print(f"Helper: Expected shape ({self.num_proto_channels}, H_proto, W_proto). Mask generation might fail.")
             # Set placeholder dimensions, but mask generation using these will likely be incorrect
             # <<< ACTION REQUIRED: VERIFY THESE FALLBACK DIMENSIONS OR FIX SEGMENTATION OUTPUT SHAPE >>>
             proto_height, proto_width = 160, 160 # Fallback placeholder for H_proto, W_proto
        ...
        #    The 2100 columns likely correspond to the total grid cells flattened across P3, P4, P5 strides.
        #    The 38 rows contain the prediction attributes (5 + num_classes + num_mask_coeffs = 39) flattened or structured in a way that results in 38.
        #    You need to know how to index/slice this tensor to get the predictions for each grid cell and anchor.
        #    The logic might involve iterating through the 2100 columns and for each column, interpreting the 38 values.
        ...
        #    - You need the correct anchor box values (self.anchor_boxes).
        #    - You need the grid cell coordinates (cx, cy) for each cell at each stride level.
        # 3. How to access the raw tx, ty, tw, th, objectness, class scores, and mask coefficients (32 values) for each prediction within the (38, 2100) tensor based on your indexing logic.
        # 4. Applying sigmoid to objectness score and class scores.
        # 5. Finding the class_id (index of the max class score).
        ...
        print("Helper: Running placeholder for detection decoding. This does NOT decode the (38, 2100) tensor correctly.")
        print("Helper: You MUST replace this entire 'Step 2' section with your actual YOLOv11 decoding logic.")
        print("Helper: The (38, 2100) shape is unusual and needs specific decoding logic from your converter.")
        ...
        # Format: (x1_320, y1_320, x2_320, y2_320, conf, class_id, mask_coeffs_np)
        ...
        potential_detections_scaled_320 = [
            ...
            (int(182.67 * scale_orig_to_320), int(229.48 * scale_orig_to_320), int(207.00 * scale_orig_to_320), int(257.75 * scale_orig_to_320), 0.5832, 0, dummy_mask_coeffs_list[2]), # Scaled Colab object 2
        ...
        # --- >>> YOUR NMS IMPLEMENTATION GOES HERE (Usually uses cv2.dnn.NMSBoxes) <<< ---
        # Use cv2.dnn.NMSBoxes on the bounding boxes (elements 0-3).
        ...
        scale_x_orig = original_width / self.input_size # e.g., 640 / 320 = 2
        scale_y_orig = original_height / self.input_size # e.g., 640 / 320 = 2
        ...
        proto_height, proto_width = 160, 160 # <<< ACTION REQUIRED: VERIFY H_proto, W_proto from segmentation_output_np.shape[1:3] >>>
        if segmentation_output_np.ndim >= 3 and segmentation_output_np.shape[0] == self.num_proto_channels:
             proto_height = segmentation_output_np.shape[1]
             proto_width = segmentation_output_np.shape[2]
             print(f"Helper: Using prototype spatial dimensions: {proto_width}x{proto_height}")
        else:
             print(f"Helper: Warning: Unexpected segmentation output shape {segmentation_output_np.shape}. Expected ({self.num_proto_channels}, H_proto, W_proto). Mask generation might fail.")
             print("Helper: Using fallback placeholder dimensions for mask generation.")
             # Use placeholder dimensions, but mask generation using these will likely be incorrect
             # <<< ACTION REQUIRED: VERIFY THESE FALLBACK DIMENSIONS OR FIX SEGMENTATION OUTPUT SHAPE >>>
             proto_height, proto_width = 160, 160 # Fallback placeholder


        # Reshape prototype masks for matrix multiplication (num_prototypes, H_proto * W_proto) -> (H_proto * W_proto, num_prototypes)
        # Check if segmentation_output_np has the expected number of prototype channels and dimensions
        prototypes_reshaped = None # Initialize as None
        if segmentation_output_np.ndim >= 3 and segmentation_output_np.shape[0] == self.num_proto_channels:
             try:
                prototypes_reshaped = segmentation_output_np.reshape(self.num_proto_channels, proto_height * proto_width).T # Shape (H_proto*W_proto, num_prototypes=256)
                 ...
                 print(f"Helper: Error reshaping prototype masks: {e}. Check segmentation output shape {segmentation_output_np.shape}.")
                 # prototypes_reshaped remains None
        # else the warning about unexpected shape is already printed above
        ...
        for det_320 in detections_after_nms_scaled_320:
            ...
            # Check if mask_coeffs are present and have the correct format
            ...
            x1_320, y1_320, x2_320, y2_320, conf, class_id, mask_coeffs = det_320 # mask_coeffs is expected to be np.ndarray (32,)
            ...
            if prototypes_reshaped is None:
                 print("Helper: Warning: Prototype masks not correctly prepared or reshaped. Cannot generate mask.")
                 ...
                 try:
                     ...
                     # Placeholder raw mask calculation (replace with real calculation):
                     print("Helper: Using placeholder raw mask calculation (replace this).")
                     # This calculates a dummy mask. You MUST replace this.
                     raw_mask_flattened = np.random.rand(proto_height * proto_width).astype(np.float32) # Dummy flattened mask
                     raw_mask = raw_mask_flattened.reshape(proto_height, proto_width) # Reshape dummy mask
    ...
    def predict(self, image_bgr_np, conf_threshold):
        ...
        # Using 'with' statement for extractor to ensure resources are managed
